...
System.out.println("Cinco Episodios mais bem avaliados: ");
        ...
        List<Episode> episodes = seasons.stream()
                ...
                System.out.println("Digite um trcho do título para buscar o episódio:");
                var chunkTittle = readScanner.nextLine();
                Optional<Episode> episodeSearch = episodes.stream()
                        .filter(e -> e.getTitle().toUpperCase().contains(chunkTittle.toUpperCase()))
                        .findFirst();

                if(episodeSearch.isPresent()) { 
                    System.out.println("Episódio selecionado: " + episodeSearch.get().getSeason());
                }               
